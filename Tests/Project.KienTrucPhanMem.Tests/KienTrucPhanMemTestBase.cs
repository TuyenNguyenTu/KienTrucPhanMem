...
using Project.KienTrucPhanMem.Migrations.SeedData;
...
namespace Project.KienTrucPhanMem.Tests
{
    public abstract class KienTrucPhanMemTestBase : AbpIntegratedTestBase<KienTrucPhanMemTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected KienTrucPhanMemTestBase()
        {
        ...
        protected void UsingDbContext(Action<KienTrucPhanMemDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<KienTrucPhanMemDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<KienTrucPhanMemDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<KienTrucPhanMemDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<KienTrucPhanMemDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<KienTrucPhanMemDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<KienTrucPhanMemDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<KienTrucPhanMemDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<KienTrucPhanMemDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<KienTrucPhanMemDbContext, Task<T>> func)
        {
            T result;
